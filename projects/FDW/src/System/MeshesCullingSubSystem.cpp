...
MeshesCullingSubSystem::MeshesCullingSubSystem(ID3D12Device* device) {
	Init(device);
}

bool MeshesCullingSubSystem::CheckFrustumCulling(CameraFrustum fr, const MeshAABBInstanceData& data) {
	return fr.IsAABBInsideFrustum(data.MinP, data.MaxP);
}

void MeshesCullingSubSystem::ProcessGPUCulling(const InputMeshesCullingProcessData& data) {
...
void MeshesCullingSubSystem::UpdateHiZResource(FD3DW::DepthStencilView* mainDSV, ID3D12Device* device, ID3D12GraphicsCommandList* list)
{
...
FD3DW::StructuredBuffer* MeshesCullingSubSystem::GetResultBuffer() {
	return m_pOutputCommandsBuffer.get();
}

void MeshesCullingSubSystem::Init(ID3D12Device* device) {
	m_pInstancesDataBuffer = FD3DW::StructuredBuffer::CreateStructuredBuffer<MeshAABBInstanceData>(device, 1, true);
	m_pCullingCameraBuffer = FD3DW::UploadBuffer<MeshesCullingCameraData>::CreateConstantBuffer(device, 1);
...
void MeshesCullingSubSystem::RecreateOutputCommandBuffer(ID3D12Device* device, ID3D12GraphicsCommandList* list, UINT count) {
	if (m_pOutputCommandsBuffer && FD3DW::StructuredBuffer::CalculateElementsCount(CountBufferOffset(count), (UINT)sizeof(IndirectMeshRenderData)) == m_pOutputCommandsBuffer->GetCapacity()) {
		m_pOutputCommandsBuffer->Clear(device, list, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE);
		return;
	}

	if (!m_pOutputCommandsBuffer) {
		m_pOutputCommandsBuffer = FD3DW::StructuredBuffer::CreateStructuredBuffer(device, CountBufferOffset(count) + (UINT)sizeof(UINT), (UINT)sizeof(IndirectMeshRenderData), true, D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS);
	}
	else {
		m_pOutputCommandsBuffer->ReserveSize(device, CountBufferOffset(count) + (UINT)sizeof(UINT), (UINT)sizeof(IndirectMeshRenderData));
	}

	m_pPack->AddResource(GetDefaultDescriptor(count), OBJECT_CULLING_OUTPUT_COMMANDS_POS_IN_PACK, device);
}

void MeshesCullingSubSystem::LoadDataToInstancesBuffer(ID3D12Device* device, ID3D12GraphicsCommandList* list, std::vector<MeshAABBInstanceData> data) {
	m_pInstancesDataBuffer->UploadData(device, list, data.data(), (UINT)data.size(), D3D12_RESOURCE_STATE_ALL_SHADER_RESOURCE);
}

void MeshesCullingSubSystem::LoadDataToCameraBuffer(CameraFrustum frustum, UINT instancesCount) {
	MeshesCullingCameraData ccs;
...
FD3DW::UAVResourceDesc MeshesCullingSubSystem::GetDefaultDescriptor(UINT commandsNum) {
	...
	desc.Stride = sizeof(IndirectMeshRenderData);

	return desc;
}

UINT MeshesCullingSubSystem::CountBufferOffset(UINT count) {
	return FD3DW::AlignForUavCounter(count * sizeof(IndirectMeshRenderData));
}
